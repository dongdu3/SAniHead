...
class DataFetcher(threading.Thread):
	def __init__(self, in_root, gt_root, file_list):
		...
		self.index = 0
		self.number = len(self.flist)

	def work(self, idx):
		name = self.flist[idx]
		in_pc = read_ply(os.path.join(self.in_root, name+'_recon_vdis.ply'))
		gt_pc = read_ply(os.path.join(self.gt_root, name+'.ply'))

		return in_pc, gt_pc, name
	
	def run(self):
		while self.index < 9000000000 and not self.stopped:
