...
class Model(object):
    def __init__(self, **kwargs):
        ...
        self.model_path = os.path.join(current_path, 'checkpoint')
...
class SAniHead(Model):
    def __init__(self, sess=None, init_infor_file='utils/sphere/init_info.dat', img_size=224, epochs=50, hidden=192,
                 feat_dim=1923, coord_dim=3, learning_rate=1e-4, weight_decay=5e-6, sketch_root=None, pc_root=None,
    ...
    def _init_placeholders(self):
        ...
        self.pool_idx = [tf.constant(pool_idx[i], dtype=tf.int32, name='pool_idx' + str(i)) for i in
                         range(len(pool_idx))]
        self.lape_idx = [tf.constant(lape_idx[i], dtype=tf.int32, name='lape_idx' + str(i)) for i in
                         range(len(lape_idx))]
    ...
    def _loss(self):
        ...
        '''
        self.mesh_loss1 += mesh_loss(self.vert1, self.gt_pcn, self.edges, 1)
        self.mesh_loss2 += mesh_loss(self.vert2, self.gt_pcn, self.edges, 2)
        self.mesh_loss3 += mesh_loss(self.vert3, self.gt_pcn, self.edges, 3)

        self.total_loss = self.mesh_loss1 + self.mesh_loss2 + self.mesh_loss3
        self.total_loss += .3 * laplace_loss(self.inputs, self.vert1, self.lape_idx, 1)
        self.total_loss += laplace_loss(self.vert1_2, self.vert2, self.lape_idx, 2)
        self.total_loss += laplace_loss(self.vert2_2, self.vert3, self.lape_idx, 3)

        # Weight decay loss
        conv_layers = range(1, 15) + range(17, 31) + range(33, 48)
        for layer_id in conv_layers:
            for var in self.layers_p2m[layer_id].vars.values():
                self.total_loss += self.weight_decay * tf.nn.l2_loss(var)

    def _build(self):
        self.build_cnn18()

        # first project block
        self.layers_p2m.append(GraphProjection(img_feat=self.img_feat, name='pixel2mesh'))
    ...
    def build_cnn18(self):
        ...
        self.total_loss += tf.add_n(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)) * 0.3
