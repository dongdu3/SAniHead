...
void CBaseModel::LoadModel(const trimesh::TriMesh &mesh)
{
	if ( mesh.vertices.size()!=0 && mesh.faces.size()!=0 && mesh.normals.size()!=0 && mesh.vertices.size()==mesh.normals.size() )
	{
		int num_vertices = mesh.vertices.size();
		int num_faces = mesh.faces.size();
		m_Verts.resize(num_vertices);
		m_NormalsToVerts.resize(num_vertices);
		m_Faces.resize(num_faces);

		for ( int i=0; i<num_vertices; ++i )
		{
			m_Verts[i] = CPoint3D(mesh.vertices[i][0],mesh.vertices[i][1],mesh.vertices[i][2]);
			m_NormalsToVerts[i] = CPoint3D(mesh.normals[i][0],mesh.normals[i][1],mesh.normals[i][2]);
		}

		for ( int i=0; i<num_faces; ++i )
		{
			m_Faces[i] = CFace(mesh.faces[i][0],mesh.faces[i][1],mesh.faces[i][2]);
		}
