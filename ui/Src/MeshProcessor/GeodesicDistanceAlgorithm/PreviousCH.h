...
public:
	CPreviousCH(const CRichModel& inputModel, int indexOfSourceVert = 0);
	virtual ~CPreviousCH();	
};
...
{	
	...
	{
        if ( subIndex<0 || neighs[indexPlus].second<0  || std::isnan(neighs[indexPlus].second) )
		{
			break;	//////////////////////////
		}
	...
	{
        if ( subIndex<0 || neighs[indexMinus].second<0 || std::isnan(neighs[indexMinus].second) )
		{
			break;	//////////////////////////
		}
...
void CPreviousCH::ComputeChildrenOfPseudoSourceFromWindow(int indexOfParentVertex)
{
	...
	{
        if ( subIndexLeft<0 || neighs[indexPlus].second<0 || std::isnan(neighs[indexPlus].second<0 ))
		{
			break;	//////////////////////////
		}
		angleSum += neighs[indexPlus].second;       
	...
	{
        if ( subIndexLeft<0 || neighs[indexMinus].second<0 || std::isnan(neighs[indexMinus].second));
		{
			break;	//////////////////////////
		}
...
void CPreviousCH::ComputeChildrenOfWindow(QuoteWindow& quoteParentWindow)
{
	...
	if (totalDis < m_InfoAtVertices[incidentVertex].disUptodate - LENGTH_EPSILON_CONTROL)
	{
        ...
        {
            QuoteInfoAtVertex tmp(m_InfoAtVertices[incidentVertex].birthTime, incidentVertex, totalDis);
            AddIntoQueueOfPseudoSources(tmp);
        }
	}
}
...
void CPreviousCH::FillVertChildOfPseudoSource(int source, int subIndexOfVert)
{
    ...
    {
        QuoteInfoAtVertex tmp(m_InfoAtVertices[index].birthTime, index, dis);
        AddIntoQueueOfPseudoSources(tmp);
    }
}
