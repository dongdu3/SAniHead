...
class GraphConvolution(Layer):
    ...
    def __init__(self, input_dim, output_dim, support, dropout=0., sparse_inputs=False, act=tf.nn.relu, bias=True,
...
class GraphProjection(Layer):
    ...
    def __init__(self, img_feat, **kwargs):
    ...
    def _call(self, inputs, r=0.27759, tz=-0.8):
        coord = inputs
        X = inputs[:, 0] * r
        Y = inputs[:, 1] * r
        Z = inputs[:, 2] * r + tz
        ...
        x = h / (224.0 / 56)
        y = w / (224.0 / 56)
        out1 = self.project(self.img_feat[0], x, y, 128)

        x = h / (224.0 / 28)
        y = w / (224.0 / 28)
        out2 = self.project(self.img_feat[1], x, y, 256)

        x = h / (224.0 / 14)
        y = w / (224.0 / 14)
        out3 = self.project(self.img_feat[2], x, y, 512)

        x = h / (224.0 / 7)
        y = w / (224.0 / 7)
        out4 = self.project(self.img_feat[3], x, y, 1024)
        outputs = tf.concat([coord, out1, out2, out3, out4], 1)
        return outputs
...
class GraphProjection2(Layer):
    """Graph Pooling layer."""

    def __init__(self, img_feat2, **kwargs):
        super(GraphProjection2, self).__init__(**kwargs)

        self.img_feat = img_feat2
